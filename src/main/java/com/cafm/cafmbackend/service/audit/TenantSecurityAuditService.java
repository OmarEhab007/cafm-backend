package com.cafm.cafmbackend.service.audit;

import com.cafm.cafmbackend.data.entity.AuditLog;
import com.cafm.cafmbackend.data.entity.Company;
import com.cafm.cafmbackend.data.repository.AuditLogRepository;
...
public class TenantSecurityAuditService {
    ...
    private UUID getCurrentUserId() {
        try {
            Authentication auth = SecurityContextHolder.getContext().getAuthentication();
            if (auth != null && auth.isAuthenticated() && 
                auth.getPrincipal() instanceof com.cafm.cafmbackend.data.entity.User user) {
                return user.getId();
            }
        } catch (Exception e) {
